...
function Illuminate(pad,velocity,time) {
    time = time || 500;
    var nPos = {n:(255*velocity) };
    pad.Tween = new TWEEN.Tween(nPos);
    pad.Tween.to({ n: 0  }, time*1000);
    pad.Tween.start();

    pad.Tween.onUpdate(function() {
        pad.Light = this.n;
    });
    pad.Tween.easing( TWEEN.Easing.Sinusoidal.InOut );
}
